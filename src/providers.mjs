...
async function main() {
  ...
  if (command === "login") {
    ...
    process.stdout.write(`${provider.displayName} sign-in completed.\n`);
    return;
  }
  if (command === "disconnect") {
    ...
    process.stdout.write(
      `${provider.displayName} disconnected; its session and picker selection were removed.\n`,
    );
    return;
  }
  if (!provider || !["enable", "disable"].includes(command)) {
    throw new Error(
      "Usage: providers [list [--json]|login antigravity-oauth|disconnect antigravity-oauth|enable ID|disable ID|generic ...]",
    );
  }
  if (command === "enable" && !configured(provider)) {
    ...
    const setup = provider.kind === "oauth"
      ? SIGN_IN_STATUS[provider.id]?.setup || oauthStatus?.setup || "sign in with the provider CLI"
      : keySetup;
